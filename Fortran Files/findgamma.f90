...
subroutine findgamma(cvt, lvt, mvt, base_num, gammatensor) ! Calculates the gamma tensor.
  ...
  integer :: i, j, k
  real(dp) :: gaunt_coeff
  double precision, intent(out) :: gammatensor(base_num, base_num, base_num)
  ...
  do i=1, size(lvt)
      do j=1, size(lvt)
          do k=1, size(lvt)
              call unit_real_gaunt(lvt(i), lvt(j), lvt(k), mvt(i), mvt(j), mvt(k), gaunt_coeff)
              gammatensor(k, j, i) = (gaunt_coeff*sqrt(2.0_dp)*gamma(0.5_dp*(lvt(i)+lvt(j)+lvt(k))+1.5_dp)*&
                                      ((2.0_dp*cvt(i))**(0.5_dp*lvt(i)+0.75_dp)*(2.0_dp*cvt(j))**(0.5_dp*&
                                      lvt(j)+0.75_dp)*(2.0_dp*cvt(k))**(0.5_dp*lvt(k)+0.75_dp)))/((cvt(k)&
                                      +cvt(j)+cvt(i))**(0.5_dp*(lvt(k)+lvt(j)+lvt(i))+1.5_dp)&
                                      *sqrt(gamma(lvt(k)+1.5_dp)*gamma(lvt(j)+1.5_dp)*gamma(lvt(i)+1.5_dp)))
          end do
      end do
  end do
